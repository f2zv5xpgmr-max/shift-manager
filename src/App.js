...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#f0f2f7", fontFamily: "'Noto Sans JP','Hiragino Kaku Gothic ProN',sans-serif" }}>
      <style>{`
      ...
      <div style={{ padding: "8px 10px", paddingBottom: "max(40px, env(safe-area-inset-bottom))" }}>
        {view === "calendar" && (
          <>
                ...
                <thead>
                  ...
                  <tr>
                    <td style={{ padding: "3px 7px", background: "#eef1f8", fontSize: 10, color: "#888",
                      fontWeight: 700, borderBottom: "2px solid #d0d8ee", position: "sticky", left: 0, zIndex: 2 }}>牧場<br/>確認</td>
                    {days.map(d => (
                      <td key={d} style={{ padding: "2px 1px", background: "#eef1f8", borderBottom: "2px solid #d0d8ee" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: 1, alignItems: "center" }}>
                          {LOCATIONS.map(l => (
                            <div key={l.id} style={{ width: 26, height: 7, borderRadius: 2,
                    ...
                    return (
                      <tr key={s.id} style={{ background: si % 2 === 0 ? "#fff" : "#fafbfd" }}>
                        <td style={{ padding: "4px 7px", fontWeight: 600, color: "#1a2235",
                          ...
                          {editingId === s.id ? (
                            <input value={editingName} onChange={e => setEditingName(e.target.value)}
                              onBlur={() => { setStaff(prev => prev.map(x => x.id === s.id ? { ...x, name: editingName } : x)); setEditingId(null); }}
                              autoFocus style={{ border: "1px solid #4f86c6", borderRadius: 4, padding: "2px 4px", fontSize: 12, width: 56 }} />
                          ) : (
                            <span onDoubleClick={() => { setEditingId(s.id); setEditingName(s.name); }} style={{ cursor: "text" }}>{s.name}</span>
                          )}
                        </td>
                        {days.map(d => {
                            ...
                            <td key={d} style={{ padding: "2px 1px", borderBottom: "1px solid #eef0f6" }}>
                              {hol ? (
                                <div className="cell" onClick={() => openModal(s.id, d)}
                                  ...
                                  <span style={{ fontSize: 9, fontWeight: 700, color: hol === "paid" ? "#e05a5a" : "#999" }}>
                                    {hol === "paid" ? "有休" : "休日"}
                                  </span>
                                </div>
                              ) : (
                                <div className={`cell${dayShifts.length === 0 ? " empty" : ""}`} onClick={() => openModal(s.id, d)}>
                                  {dayShifts.length === 0 ? "+" : dayShifts.map(sh => (
                                    <div key={sh.loc.id} style={{ background: sh.loc.color, color: "#fff", borderRadius: 3,
                                      padding: "1px 2px", width: "100%", textAlign: "center", fontSize: 9, fontWeight: 700, lineHeight: 1.3 }}>
                                      {sh.loc.name.slice(0, 2)}
                                    </div>
                                  ))}
                                </div>
                              )}
                        ...
                        <td style={{ padding: "4px 5px", textAlign: "center", borderBottom: "1px solid #eef0f6" }}>
                          ...
                          <div style={{ fontSize: 10, fontWeight: 700, color: holOk ? "#5cb85c" : "#e05a5a", lineHeight: 1.2 }}>
            ...
            <div style={{ marginTop: 8, color: "#aaa", fontSize: 11, textAlign: "center" }}>セルをタップ → 牧場選択 → 時間設定</div>
          </>
        )}

        {view === "summary" && (
          <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 12px #0001", overflow: "auto" }}>
            <div style={{ padding: "12px 14px 4px", fontWeight: 700, fontSize: 15, color: "#1a2235" }}>勤務集計 — {year}年{month + 1}月</div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr>
                  <th style={{ padding: "8px 10px", background: "#f5f7fc", textAlign: "left", color: "#555", borderBottom: "2px solid #e0e6f0" }}>スタッフ</th>
                  {LOCATIONS.map(l => (
                    <th key={l.id} style={{ padding: "8px 6px", background: "#f5f7fc", textAlign: "center", color: l.color, borderBottom: "2px solid #e0e6f0", fontWeight: 700, fontSize: 11 }}>{l.name}</th>
                  ))}
                  <th style={{ padding: "8px 6px", background: "#f5f7fc", textAlign: "center", color: "#e05a5a", borderBottom: "2px solid #e0e6f0", fontWeight: 700 }}>有休</th>
                  <th style={{ padding: "8px 6px", background: "#f5f7fc", textAlign: "center", color: "#888", borderBottom: "2px solid #e0e6f0", fontWeight: 700 }}>休日<br/><span style={{ fontSize: 10, fontWeight: 400 }}>({requiredHolidays}日)</span></th>
                  <th style={{ padding: "8px 6px", background: "#f5f7fc", textAlign: "center", color: "#1a2235", borderBottom: "2px solid #e0e6f0", fontWeight: 700 }}>実働</th>
                </tr>
              </thead>
              <tbody>
                {summary.map(({ s, totalWork, paidDays, byLoc, holDays, holOk }, i) => (
                  <tr key={s.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafbfd" }}>
                    ...
                    {LOCATIONS.map(l => (
                      <td key={l.id} style={{ padding: "8px 6px", textAlign: "center", borderBottom: "1px solid #eef0f6", color: byLoc[l.id] ? l.color : "#ddd", fontWeight: byLoc[l.id] ? 700 : 400 }}>
                        {byLoc[l.id] ? minutesToHHMM(byLoc[l.id]) : "—"}
                      </td>
                    ))}
                    <td style={{ padding: "8px 6px", textAlign: "center", borderBottom: "1px solid #eef0f6", color: paidDays ? "#e05a5a" : "#ddd", fontWeight: paidDays ? 700 : 400 }}>{paidDays ? `${paidDays}日` : "—"}</td>
                    <td style={{ padding: "8px 6px", textAlign: "center", borderBottom: "1px solid #eef0f6" }}>
                      <span style={{ fontWeight: 700, color: holOk ? "#5cb85c" : "#e05a5a" }}>{holOk ? "✅" : "⚠️"} {holDays}日</span>
                    </td>
                    <td style={{ padding: "8px 6px", textAlign: "center", fontWeight: 700, color: "#1a2235", borderBottom: "1px solid #eef0f6" }}>{minutesToHHMM(totalWork)}</td>
                  </tr>
                ))}
                <tr style={{ background: "#f5f7fc" }}>
                  ...
                  {LOCATIONS.map(l => {
                    ...
                    return <td key={l.id} style={{ padding: "8px 6px", textAlign: "center", fontWeight: 700, color: l.color, borderTop: "2px solid #e0e6f0" }}>{minutesToHHMM(lt)}</td>;
                  })}
                  <td style={{ padding: "8px 6px", textAlign: "center", fontWeight: 700, color: "#e05a5a", borderTop: "2px solid #e0e6f0" }}>{summary.reduce((a, { paidDays }) => a + paidDays, 0) || "—"}{summary.reduce((a, { paidDays }) => a + paidDays, 0) ? "日" : ""}</td>
                  <td style={{ borderTop: "2px solid #e0e6f0" }} />
                  <td style={{ padding: "8px 6px", textAlign: "center", fontWeight: 700, fontSize: 14, color: "#1a2235", borderTop: "2px solid #e0e6f0" }}>{minutesToHHMM(summary.reduce((a, { totalWork }) => a + totalWork, 0))}</td>
                </tr>
              </tbody>
            </table>
            <div style={{ padding: "8px 14px 12px", fontSize: 11, color: "#aaa" }}>※ 実働時間は休憩を差し引いた時間 ／ 月{REQUIRED_WORK_DAYS}日勤務</div>
          </div>
        )}
      </div>

      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "#0007", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }}
          onClick={e => { if (e.target === e.currentTarget) setModal(null); }}>
          <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", padding: "20px 20px",
            paddingBottom: "max(32px, env(safe-area-inset-bottom))", width: "100%", maxWidth: 500, boxShadow: "0 -8px 40px #0003" }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 15, color: "#1a2235", marginBottom: 4 }}>
              {modalStaff?.name} ／ {month + 1}月{modal.day}日({DAYS_JP[new Date(year, month, modal.day).getDay()]})
            </div>
            {(getShiftsForDay(modal.staffId, modal.day).length > 0 || modalHoliday) && step === "top" && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                {modalHoliday && <span style={{ background: modalHoliday === "paid" ? "#e05a5a" : "#aaa", color: "#fff", borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 700 }}>{modalHoliday === "paid" ? "有休" : "休日"}</span>}
                {getShiftsForDay(modal.staffId, modal.day).map(sh => (
                  <span key={sh.loc.id} style={{ background: sh.loc.color, color: "#fff", borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 600 }}>{sh.loc.name}｜{sh.start}〜{sh.end}</span>
                ))}
              </div>
            )}
            {step === "top" && (
              <>
                  ...
                  {LOCATIONS.map(l => {
                    ...
                    return (
                      <button key={l.id} className="big-btn" onClick={() => selectLocation(l.id)}
                        style={{ background: l.color, color: "#fff", position: "relative", boxShadow: existing ? `0 0 0 3px ${l.color}55` : "none" }}>
                        {covered && !existing && <span style={{ position: "absolute", top: 7, left: 9, fontSize: 11 }}>✅</span>}
                        {l.name}
                        <div style={{ fontSize: 10, fontWeight: 400, marginTop: 3, opacity: 0.88 }}>{existing ? `${existing.start}〜${existing.end}` : `${l.start}〜${l.end}`}</div>
                        {existing && <span style={{ position: "absolute", top: 7, right: 9, background: "#fff4", borderRadius: 8, padding: "1px 6px", fontSize: 10 }}>編集</span>}
                      </button>
                    );
                  })}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                  <button className="big-btn" onClick={() => saveHoliday("off")}
                    style={{ background: modalHoliday === "off" ? "#aaa" : "#f0f0f0", color: modalHoliday === "off" ? "#fff" : "#666", border: modalHoliday === "off" ? "none" : "1.5px solid #ddd" }}>
                    😴 休日{modalHoliday === "off" && <div style={{ fontSize: 10, fontWeight: 400, marginTop: 2 }}>設定済み</div>}
                  </button>
                  <button className="big-btn" onClick={() => setStep("holiday")}
                    style={{ background: modalHoliday === "paid" ? "#e05a5a" : "#fff0f0", color: modalHoliday === "paid" ? "#fff" : "#e05a5a", border: modalHoliday === "paid" ? "none" : "1.5px solid #f5c0c0" }}>
                    📋 有休{modalHoliday === "paid" && <div style={{ fontSize: 10, fontWeight: 400, marginTop: 2 }}>設定済み</div>}
                  </button>
                </div>
                {(modalHoliday || getShiftsForDay(modal.staffId, modal.day).length > 0) && (
                  <button onClick={() => clearDay(modal.staffId, modal.day)}
                    style={{ width: "100%", padding: "10px 0", borderRadius: 10, marginBottom: 8, border: "1.5px solid #e0e6f0", background: "#fff", color: "#e05a5a", fontWeight: 600, cursor: "pointer", fontSize: 13 }}>
                    🗑 この日の設定をすべて削除
                  </button>
                )}
                <button onClick={() => setModal(null)} style={{ width: "100%", padding: "10px 0", borderRadius: 10, border: "1.5px solid #e0e6f0", background: "#fff", color: "#888", fontWeight: 600, cursor: "pointer", fontSize: 13 }}>キャンセル</button>
              </>
            )}​​​​​​​​​​​​​​​​
